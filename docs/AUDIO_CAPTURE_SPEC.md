...
Wave 1: `recorder_stop_fixture` still encrypts `CONSULT-001.wav` for `make demo`. The desk Record button uses `recorder_stop` + microphone chunks (`live-record`). System-audio uses ScreenCaptureKit when Screen Recording is already granted; otherwise `CAPTURE_UNSUPPORTED` / `needs-permission`. Mixed requires both the tap and the mic; it never records mic-only. Tests never prompt.
